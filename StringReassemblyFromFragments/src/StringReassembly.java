...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //take equation directly from contract to get the modified str1
        String newString1 = str1.substring(0, str1.length() - overlap);
        //add str2 to the new string to get new overlapped string
        return newString1 + str2;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //create a boolean that becomes false if str isn't found in the set
        //if it isn't found in the set, nothing will be done and the set
        //won't go through the rest of the method
        boolean foo = true;
        //loop will go through and see if str is found in any string
        for (String s : strSet) {
            if (s.indexOf(str) != -1) {
                foo = false;
            }
        }
        //if str is found anywhere, the rest of the method will be executed
        if (foo) {
            //add str to the set so it can be compared to str
            strSet.add(str);
            //create a temp set that will have the final set
            Set<String> temp = new Set1L<>();
            //for loop will add Strings to str if it isn't a duplicate
            for (String s : strSet) {
                if (s.equals(str)) {
                    temp.add(s);
                    //if str isn't found, check if s isn't found in str
                } else {
                    if (str.indexOf(s) == -1) {
                        temp.add(s);
                    }
            ...
            //transfer the temporary set into the set that needs to be modified
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> setOfLines = new Set1L<>();
        while (!input.atEOS()) {
            //read lines from file
            String line = input.nextLine();
            //add to the set while avoiding substrings
            addToSetAvoidingSubstrings(setOfLines, line);
        }
        return setOfLines;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //create a loop to search through every character of text
        //and find any that are equal to '~'
        for (int i = 0; i < text.length(); i++) {
            char c = text.charAt(i);
            // if the char is '~', print a new line
            if (c == '~') {
                out.println("");
                //if it is anything else, print it on the same line
            } else {
                out.print(c);
            }
        }
