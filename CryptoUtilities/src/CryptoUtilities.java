...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        //n will stay the same if m is zero, but if not, n will equal m and m
        //will equal n % m
        while (!m.isZero()) {
            //divide method will give us n % m because it returns the remainder
    ...
    public static boolean isEven(NaturalNumber n) {
        boolean even = false;
        //if last digit is even, it's automatically even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        //take fast power method from lab and add parameter m to it
        // anything to the power of 0 is 1, so set n equal to 1 if power is 0
        NaturalNumber one = new NaturalNumber2(1);
        //create copies for n and p so their values won't be changed in multiplication
        NaturalNumber copyN = new NaturalNumber2(n);
        NaturalNumber copyP = new NaturalNumber2(p);
        if (p.compareTo(new NaturalNumber2(0)) == 0) {
            n.copyFrom(one);
        } else {
            //if n is even, do fast power the normal way
            if (isEven(n)) {
                copyP.divide(new NaturalNumber2(2));
                powerMod(copyN, copyP, m);
                copyN.multiply(copyN);
            } else {
                //if n is odd, we must subtract 1 from the power and multiply
                //the number we get by the original n
                copyP.decrement();
                powerMod(copyN, copyP, m);
                copyN.multiply(n);
            }
            //must "reduce the result by modulo m" divide will give us the remainder
            n.transferFrom(copyN.divide(m));
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //create test variables for w and n so their values won't change
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber testW = new NaturalNumber2(w);
        NaturalNumber testW1 = new NaturalNumber2(w);
        //new n for w^ n-1
        NaturalNumber testN = new NaturalNumber2(n);
        testN.decrement();
        //witness will be true if w^2 % n = 1
        //witness will also be true if w^n-1 & n != 1
        //use OR (||) in if-statement
        powerMod(testW, new NaturalNumber2(2), new NaturalNumber2(n));
        powerMod(testW1, testN, new NaturalNumber2(n));
        if (testW.compareTo(one) == 0 || testW1.compareTo(one) != 0) {
            witness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
         ...
         */
        //create a foor loop to go through 50 candidates
        for (int i = 0; i < 50; i++) {
            //use a random number for witness to compositeness method
            NaturalNumber rand = new NaturalNumber2(randomNumber(n));
            if (isWitnessToCompositeness(rand, n)) {
                //return false if one candidate is witness
                //to n being composite
                //(couldn't figure out how to end the loop without
                //using 2 return statements)
                return false;
            }
        }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";
        //create a random number for witnessToCompositeness method
        NaturalNumber rand = new NaturalNumber2(randomNumber(n));
        // While loop to generate next likely prime
        while (!isWitnessToCompositeness(rand, n)) {
            //if n is even, add one and see if it is prime after
            if (isEven(n)) {
                n.increment();
                // if n is already odd, add two more and check again
            } else {
