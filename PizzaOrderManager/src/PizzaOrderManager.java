...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            String strPrice = line.substring(line.indexOf(",") + 1,
                    line.length());
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        price = price + size;
        String strTop = input.nextLine();
        int topping = 0;
        while (!strTop.equals("")) {
            topping = toppingPriceMap.value(strTop);
            price = price + topping;
            strTop = input.nextLine();
        }
        return price;
    }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        if (price < 100) {
            output.print("$0." + price);
        }
        if (price < 1000) {
            output.print("$" + priceStr.charAt(0) + "."
                    + priceStr.substring(1, priceStr.length()));
        } else {
            output.print("$" + priceStr.substring(0, 2) + "."
